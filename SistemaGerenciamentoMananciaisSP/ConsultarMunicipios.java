...
public class ConsultarMunicipios extends JFrame{
   //atributos
   ////////////////////////
   ...
   private ArrayList<Municipios> lista;
   private Connection conn;
   private JTextArea txtDescricao;
      
   public ConsultarMunicipios(Connection conn){
      //titulo do frame
      ...
      lblImagem.setPreferredSize(new Dimension(200, 100));
      ...
      txtDataInicial.setPreferredSize(new Dimension(2, 20));
      txtDataFinal = new JTextField(8);
      txtDataFinal.setPreferredSize(new Dimension(2, 20));
   ...
   public void cbActionPerformed(ActionEvent e){
      municipios.setNome((String)cb.getSelectedItem());
      
      ArrayList<Municipios> list = new ArrayList<>();
      list = municipios.carregar(conn);
            
      String m = "";
      int i = 0;
      for(Municipios n:list){
         m = m+"\n"+i+"-"+n.getNomeSistema();
         i=i+1;
      }
      int menu = -1;
      ...
      do{
         try{
            menu = Integer.parseInt(JOptionPane.showInputDialog("Confirme o sistema que deseja consultar:\n"+m));
         }
         catch(Exception ex){}
      }while(menu < 0 || menu >= list.size());
         if(menu >= 0 && menu < list.size()){
            String selecionado = ""+list.get(menu).getNomeSistema();
            municipios.setNomeSistema(selecionado);
         }
      }    
      if(municipios.getNomeSistema().equals("Cantareira")){
      ...
      if(municipios.getNomeSistema().equals("Alto Tietê")){
         pnlConteudo.remove(lblNomeSistema);
         pnlMeio.remove(lblRelatorio);
         pnlMeio.remove(lblImagem);
         pnlConteudo.remove(sPDescricao);
      ...
      if(municipios.getNomeSistema().equals("Guarapiranga")){
         pnlConteudo.remove(lblNomeSistema);
         pnlMeio.remove(lblRelatorio);
         pnlMeio.remove(lblImagem);
         pnlConteudo.remove(sPDescricao);
      ...
      if(municipios.getNomeSistema().equals("Alto Cotia")){
         pnlConteudo.remove(lblNomeSistema);
         pnlMeio.remove(lblRelatorio);
         pnlMeio.remove(lblImagem);
         pnlConteudo.remove(sPDescricao);
      ...
      if(municipios.getNomeSistema().equals("Rio Grande")){
         pnlConteudo.remove(lblNomeSistema);
         pnlMeio.remove(lblRelatorio);
         pnlMeio.remove(lblImagem);
         pnlConteudo.remove(sPDescricao);
      ...
      if(municipios.getNomeSistema().equals("Rio Claro")){
         pnlConteudo.remove(lblNomeSistema);
         pnlMeio.remove(lblRelatorio);
         pnlMeio.remove(lblImagem);
         pnlConteudo.remove(sPDescricao);
